...
// Curated list of popular exercises for women
// Organized by primary muscle group
const CURATED_EXERCISES: { name: string; muscle: string; equipment: string }[] = [
  // GLUTES (35 exercises)
  ...
  { name: 'pendulum squat', muscle: 'quads', equipment: 'leverage machine' },
  ...
  { name: 'meadows row', muscle: 'upper back', equipment: 'barbell' },
  { name: 'pendlay row', muscle: 'upper back', equipment: 'barbell' },
  ...
  { name: 'smith machine bench press', muscle: 'chest', equipment: 'smith machine' },
  ...
  { name: 'handstand push up', muscle: 'shoulders', equipment: 'body weight' },
  ...
  { name: 'machine bicep curl', muscle: 'biceps', equipment: 'leverage machine' },
  ...
  { name: 'diamond push up', muscle: 'triceps', equipment: 'body weight' },
  ...
  { name: 'captain chair leg raise', muscle: 'abs', equipment: 'body weight' },
  ...
  { name: 'decline sit up', muscle: 'abs', equipment: 'body weight' },
  ...
  { name: 'smith machine calf raise', muscle: 'calves', equipment: 'smith machine' },
];

async function fetchFromAscend(query: string): Promise<AscendExercise | null> {
  try {
    ...
    const exactMatch = exercises.find((e: AscendExercise) => e.name.toLowerCase() === queryLower);

    if (exactMatch) return exactMatch;

    // Return first result if it contains all words from query
    const queryWords = queryLower.split(' ');
    const closeMatch = exercises.find((e: AscendExercise) => {
...
async function seedExercises() {
  ...
  for (let i = 0; i < CURATED_EXERCISES.length; i++) {
    ...
    const { error } = await supabase.from('exercises').insert({
      ...
      muscle: exercise.muscle, // Use our curated muscle group (more accurate)
