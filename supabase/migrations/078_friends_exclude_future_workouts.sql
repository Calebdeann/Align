-- =============================================
-- MIGRATION 078: Exclude future-dated workouts from get_friends_with_activity
-- =============================================
-- The seed script (scripts/seed-fake-workouts.mjs) creates ~30 days of
-- workouts per fake buddy, dated from yesterday forward — most are in the
-- FUTURE so the Discover feed can drip them out over time.
--
-- The Friends-feed RPC was picking each friend's latest workout regardless of
-- date, which meant a seeded buddy's "latest" was always weeks in the future.
-- That made `is_active` true (future > today_start), made the client think
-- the workout happened "Just now" (negative time diff), and produced a blank
-- workout-summary on tap (get_public_workout_details filters completed_at <= now).
--
-- Fix: add `w.completed_at <= NOW()` to the latest_workouts CTE so the RPC
-- only considers actual past workouts. The Friends feed now correctly shows
-- a friend's most-recent *past* workout, not their queued drip releases.
