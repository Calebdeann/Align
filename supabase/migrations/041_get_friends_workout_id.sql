-- Add workout_id to get_friends_with_activity so clients can navigate to a
-- friend's specific workout detail screen.
-- Must drop first because the return type is changing (adding workout_id column).
