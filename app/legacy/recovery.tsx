...
import BodyGraph from '@/components/recovery/BodyGraph';
import MuscleBarList from '@/components/recovery/MuscleBarList';
...
export default function RecoveryScreen() {
  ...
  const fetchAllData = useCallback(
    async (isManualRefresh = false) => {
      if (!userId) return;
      // Prevent concurrent fetches — only allow if manually refreshing
      ...
      try {
        const [todayData, weekData, monthData, todayStats, weekStats, monthStats] =
          await Promise.all([
  ...
  // On tab focus: only re-fetch if data is stale (5+ min) or never loaded
  useFocusEffect(
    useCallback(() => {
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>{t('recovery.muscleWorked')}</Text>
          <BodyGraph muscleData={currentData} isLoading={isLoading} />
        </View>

        {/* Main Muscle Groups Card */}
